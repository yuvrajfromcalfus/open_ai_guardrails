...
async def run_tests_input():
    ...
    with open("guardrail_test_input_guardrails_results.json", "w", encoding="utf-8") as f:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for category, prompts in TEST_OUTPUT_PROMPTS.items():
            ...
            for prompt in prompts:
                ...
                category_results.append(result)
            results["results"][category] = category_results
    with open("guardrail_test_output_guardrails_results.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
